...
void isam_imprimir_no(T_ISAM *isam, TNo_ISAM *no){
    ...
    for (int i=0;i<no->t-1;i++){
        if (no->chaves[i] != INFINITO){
            fprintf(isam->arq_log,"|c_(%3d)=%3ld|",i,no->chaves[i]);
        }
        else{
    ...
    for (int i=0;i<no->t;i++){
        fprintf(isam->arq_log,"|f_(%3d)=%3ld|",i,no->filhos[i]);
        
    } 
    if (no->tipo !=INTERNO){
        fprintf(isam->arq_log,"|p=%ld|",no->prox);
    }
    fprintf(isam->arq_log,"\n");
}
...
void isam_criar_paginas_de_dados(T_ISAM *isam){
    ...
    fprintf(isam->arq_log,"Tamanho (#registros) do arquivo de dados: %ld\n",isam->tam_arq_dados);
...
void* isam_ler_dado_chave_no_interno(T_ISAM *isam, int chave_reg){
    //Criando o espaço em que será guardado o conteúdo do ponteiro.
    void *pont = isam->criar_dado();

    // Lendo no arquivo de dados o conteúdo referente a chave_reg
    isam->ler_dado_pos(isam->arq_dados,pont,chave_reg);

    // Retornando o ponteiro do conteúdo.
    return pont;
}
...
void *isam_ler_dado_chave_no_folha(T_ISAM *isam, TNo_ISAM *no_folha, void *consulta){
    void *pont = isam->criar_dado();
    for (int i = 0; i < (no_folha->t); i++){
        isam->ler_dado_pos(isam->arq_dados, pont,no_folha->filhos[i]);
        isam->imprimir_dado(isam->arq_log,pont);
        if (isam->comparar(consulta,pont)==0){
            return pont;
        } else if (no_folha->filhos[i] == -1) return NULL;
    }
    return NULL;
}
...
void* isam_buscar(T_ISAM *isam, void *consulta){
    ...
    //Nó auxiliar para percorrer a árvore
    TNo_ISAM *no_aux = isam_criar_no(isam,INTERNO);
    isam_inicializar_no(no_aux);

    isam_ler_no_pos(isam, no_aux, isam->raiz);

    // Busca pelos nós internos
    while(no_aux->tipo == INTERNO){
        int i = 0;
        void *pont_aux = isam->criar_dado();
        pont_aux = isam_ler_dado_chave_no_interno(isam, no_aux->chaves[i]);
        while(i<(isam->t-1)){          
            isam->imprimir_dado(isam->arq_log,pont_aux);
            if (isam->comparar(pont_aux,consulta) == 0){
                isam_ler_no_pos(isam, no_aux, no_aux->filhos[i+1]);
                break;
            } else if (isam->comparar(pont_aux,consulta) > 0){
                isam_ler_no_pos(isam, no_aux, no_aux->filhos[i]);
                break;
            } else {
                i++;
                pont_aux = isam_ler_dado_chave_no_interno(isam, no_aux->chaves[i]);
    ...
    //Busca pelos nó folha.
    void *folha = isam->criar_dado();
    folha = isam_ler_dado_chave_no_folha(isam, no_aux, consulta);
    return folha;
}


int no_folha_incompleto(TNo_ISAM *no){
    for (int i = 0; i < no->t; i++){
...
int isam_insere(T_ISAM *isam,void *dado){

    // Achando a posição do dado, caso o dado já esteja na isam, é retornado nulo.
    // Nó auxiliar para percorrer a árvore
    TNo_ISAM *no_aux = isam_criar_no(isam,INTERNO);
    isam_inicializar_no(no_aux);
    ...
    int i = 0;
    while(no_aux->tipo == INTERNO){
        void *pont_aux = isam_ler_dado_chave_no_interno(isam, no_aux->chaves[i]);
        while(i<(isam->t-1)){
            if (isam->comparar(pont_aux,dado) == 0){
                isam_ler_no_pos(isam, no_aux, no_aux->filhos[i+1]);
                break;
            } else if (isam->comparar(pont_aux,dado) > 0){
                isam_ler_no_pos(isam, no_aux, no_aux->filhos[i]);
                break;
            } else {
                i++;
    ...
    // Busca pelo nó folha
    for (int j= 0; j < no_aux->t;j++){
        void *pont_aux = isam_ler_dado_chave_no_interno(isam, no_aux->filhos[j]);
        // Excluindo caso de Dado já inserido no nó folha.  
        if (isam->comparar(pont_aux, dado)==0) return 0;
        // Caso não tenha folha completa
        if(isam->comparar(pont_aux, dado) > 0 && no_folha_incompleto(no_aux)){ 
            fseek(isam->arq_dados, 0, SEEK_END);
            isam_salvar_no_pos(isam, no_aux, isam_pos_arq_ind(isam)+1);
            isam->salvar_dado_pos(isam->arq_dados,dado,isam_pos_arq_dados(isam)+1);

            // Ordenação de nó folha
            long temp1;
            long temp2 = no_aux->filhos[j];
            no_aux->filhos[j] = isam->tam_arq_dados+1;
            while (j != no_aux->t){
                temp1 = no_aux->filhos[j+1]; 
                no_aux->filhos[j+1] = temp2;
                temp2 = temp1;
                j++;
            }
            // Eu sei que tenho que atualizar a chave_reg, contudo não sei como.
            return 1;
        }
    }    
    // Caso de overflow: Não fiz.
}
