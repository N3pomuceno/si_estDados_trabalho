...
void insere_registro(T_ISAM * isam_cod,T_ISAM *isam_nome){
    ...
    //Anselmo (versao 1.1) - problema de formato corrigido na leitura do nome e salario
    printf("\nDigite:\n");
    printf("\nCodigo:");
    scanf("%d",&cod);
    printf("\nDigite o nome:");
...
void insercao(T_ISAM *isam_cod,T_ISAM *isam_nome, int cod, char nome[], float salario){
    ...
    //Anselmo (versao 1.1): checagem da insercao (para evitar salvamento duplicado) 
    if (isam_insere(isam_nome,func)){
        ...
        fprintf(isam_cod->arq_log,"Salvamento nao efetuado. Registro ja existe. \n");    
    }
...
void teste_consulta_simples_por_codigo(T_ISAM *isam_cod, T_ISAM *isam_nome){
    ...
    //Anselmo (versao 1.1) : consulta da ultima pagina
    consulta_por_cod(isam_cod,1805);
    // Exemplos extras para testar.
    consulta_por_cod(isam_cod,1000);
    consulta_por_cod(isam_cod,1020);
    consulta_por_cod(isam_cod,1155);
    consulta_por_cod(isam_cod,1083);
    consulta_por_cod(isam_cod,2879);
    consulta_por_cod(isam_cod,3672);
    consulta_por_cod(isam_cod,3008);
...
void teste_insercao_simples(T_ISAM *isam_cod, T_ISAM *isam_nome){
    ...
    //Anselmo (versao 1.1): adicionado teste de registro duplicado
    insercao(isam_cod,isam_nome,1886,"Luana Souza",10000.00f);    
    insercao(isam_cod,isam_nome,1002,"Antonio Carlos",2345.0);
